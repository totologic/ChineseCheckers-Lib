Create a project with your favorite editor (Flash Builder, Flashdevelop...)
Set game.as as your main.
Link the chineseCheckers lib folder to your project.
Link the ui.swc file to your project.
Then run.

editor.swf can be used to edit your own grid.
Run it.
Set width height and generate.
Then paint on the grid with empty, player0 ans player1 colors.
Export and copy paste in your code.
